...
fn entry(stage: RequestStage, body: &[u8]) -> CachedResponseEntry {
    CachedResponseEntry {
        stage,
        request_fingerprint: match stage {
            RequestStage::Wide => WIDE,
            RequestStage::Rerank => RERANK,
        },
...
fn seed(path: &Path) {
    drop(
        publish(
            open(path),
            NS,
            entry(RequestStage::Wide, b"old-wide"),
...
fn rows(path: &Path, namespace: [u8; 32]) -> Vec<(String, Vec<u8>)> {
    ...
    let mut query = db
        .prepare("SELECT stage,response FROM sr_cache_response WHERE namespace=?1 ORDER BY stage")
        .unwrap();
    query
...
fn old_rows() -> Vec<(String, Vec<u8>)> {
    vec![
        ("rerank".into(), b"old-rerank".to_vec()),
        ("wide".into(), b"old-wide".to_vec()),
    ]
}
...
fn acquire(path: &Path) -> (CacheStore, LeaderContext) {
    ...
    let (store, acquisition) = store
        .acquire_lease(&invocation, &cx, CoordinationKey::from_bytes([7; 32]), true)
        .unwrap();
    let LeaseAcquisition::Leading(leader) = acquisition else {
...
fn completed(path: &Path) -> bool {
    let db = rusqlite::Connection::open(path.join(CACHE_FILE)).unwrap();
    db.query_row("SELECT is_completed FROM sr_coordination_leases", [], |r| {
        r.get(0)
    })
    .unwrap()
}
...
fn pair_and_completion_survive_close_and_reopen_together() {
    ...
    drop(
        publish(
            store,
            NS,
            entry(RequestStage::Wide, b"new-wide"),
    ...
    assert_eq!(
        rows(&path, NS),
        vec![
            ("rerank".into(), b"new-rerank".to_vec()),
            ("wide".into(), b"new-wide".to_vec()),
        ]
    );
    let store = open(&path);
    let invocation = ProcessInvocation::enter().unwrap();
    ...
    let (store, wide) = store
        .response(&invocation, &cx, NS, RequestStage::Wide, WIDE)
        .unwrap();
    let (_, rerank) = store
        .response(&invocation, &cx, NS, RequestStage::Rerank, RERANK)
        .unwrap();
...
fn failed_second_row_preserves_previous_pair_and_active_lease() {
    ...
    assert_eq!(
        publish(
            store,
            NS,
            entry(RequestStage::Wide, b"never-committed"),
            Some(invalid),
    ...
    drop(
        publish(
            open(&path),
            NS,
            entry(RequestStage::Wide, b"retry-wide"),
...
fn wide_only_publication_removes_old_rerank_and_completes() {
    ...
    drop(
        publish(
            store,
            NS,
            entry(RequestStage::Wide, b"low-need"),
            None,
            Some((path.join(CACHE_FILE), leader)),
        )
        .unwrap(),
...
fn replacing_shared_rerank_key_removes_other_wide_fingerprints() {
    ...
    drop(
        publish(
            ...
            NS,
            wide,
            Some(entry(RequestStage::Rerank, b"same-key-new-rerank")),
            None,
        )
        .unwrap(),
    );
...
fn another_namespace_keeps_its_complete_pair() {
    ...
    drop(
        publish(
            open(&path),
            [8; 32],
            entry(RequestStage::Wide, b"other-wide"),
            Some(entry(RequestStage::Rerank, b"other-rerank")),
            None,
        )
        .unwrap(),
    );
...
fn busy_writer_does_not_replace_either_stage_or_complete_lease() {
    ...
    assert_eq!(
        publish(
            store,
            NS,
            entry(RequestStage::Wide, b"blocked-wide"),
...
fn completed_publisher_cannot_replace_the_pair_again() {
    ...
    assert_eq!(
        publish(
            store,
            NS,
            entry(RequestStage::Wide, b"repeat-wide"),
...
fn wrong_stage_model_and_combined_size_fail_before_replacement() {
    ...
    for wrong in 0..3 {
        ...
        assert_eq!(
            publish(open(&path), NS, wide, Some(rerank), None).unwrap_err(),
    ...
    assert_eq!(
        publish(
            ...
            NS,
            wide,
            Some(entry(RequestStage::Rerank, b"x")),
            None
        )
        .unwrap_err(),
        StoreError::Quota
...
fn wrong_store_fence_leaves_both_databases_untouched() {
    ...
    assert_eq!(
        publish(
            store,
            NS,
            entry(RequestStage::Wide, b"rejected"),
            None,
            Some((wrong.clone(), leader))
        )
        .unwrap_err(),
