...
fn assert_only_helpful(roster: &ResolvedRoster) {
    let names: Vec<_> = roster.advisory().map(|s| s.binding.invocation.as_str()).collect();
    assert_eq!(names, ["helpful"]);
    let id = &roster.advisory().next().unwrap().binding.id;
    assert_eq!(OptionMap::new(roster, std::slice::from_ref(id)).unwrap().entries().len(), 1);
}

fn assert_blocked(roster: &ResolvedRoster) {
    assert_eq!(roster.exact_name("blocked"), ExactResolution::Unverified);
    let bindings: Vec<_> = roster.skills().iter().flat_map(|s| s.bindings())
        .filter(|b| b.invocation.as_str() == "blocked").collect();
    assert!(!bindings.is_empty(), "the readable lower-priority binding remains inspectable");
...
fn oversized_first_root_cannot_starve_later_healthy_skills() {
    for size in [SKILL_FILE_BYTES.max() + 1, DISCOVERY_PARSED_BYTES.max() + 1] {
        ...
        assert_eq!(discovery.rejected_candidates()[0].problem(), CandidateProblem::Oversized);
        ...
        assert_eq!(roster.exact_name("private-size-canary"), ExactResolution::Missing);
    }
}
...
fn many_oversized_files_do_not_accumulate_a_fictitious_read_budget() {
    ...
    assert!(roster.diagnostics().iter().all(|(_, e)| *e == ResolutionError::Oversized));
}
...
fn nonregular_personal_slots_cannot_promote_the_project_definition() {
    for kind in ["fifo", "directory", "linked-fifo", "linked-directory"] {
        ...
        assert_eq!(discovery.rejected_candidates()[0].problem(), CandidateProblem::NotRegularFile);
        let roster = resolve(&plan);
        assert_only_helpful(&roster);
...
fn rejected_slots_share_the_pass_wide_entry_limit() {
    ...
    for index in 0..3 {
        ...
        let spec = RootSpec::new(
            ...
            SourceKind::Generic, 0, verified(), "SKILL.md",
        ).unwrap();
...
fn the_real_cumulative_byte_limit_still_bounds_readable_candidates() {
    ...
    assert_eq!(discovery.bytes_examined(), DISCOVERY_PARSED_BYTES.max() as u64);
...
fn unsupported_rejected_layouts_do_not_claim_callable_names() {
    ...
    assert_only_helpful(&roster);
    assert_eq!(roster.diagnostics().len(), 1);
    assert_eq!(roster.diagnostics()[0].1, ResolutionError::UnsupportedLayout);
}
...
fn moving_a_rejection_to_a_competing_name_invalidates_publication() {
    ...
    fs::rename(home.join(".claude/skills/unrelated"), home.join(".claude/skills/helpful")).unwrap();
    ...
    assert_eq!(revalidation::revalidate(&captured, &fresh, &clock).unwrap_err(), RevalidationError::Changed);
    assert!(runtime.shutdown());
}
...
fn repairing_a_rejected_override_requires_fresh_resolution_and_keeps_restrictions() {
    ...
    fs::write(&oversized, "---\ndisable-model-invocation: true\n---\nManual procedure").unwrap();
    let repaired = resolve(&plan);
    assert_only_helpful(&repaired);
    assert!(matches!(repaired.exact_name("blocked"), ExactResolution::Resolved {
