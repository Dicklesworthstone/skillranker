...
fn a_real_active_conflict_is_not_discarded_with_siblings() {
    ...
    assert!(matches!(
        resolve(&input),
        AnchorResolution::ConflictingDirectives { .. }
    ));
}
...
fn parent_order_not_input_order_selects_the_continuation_antecedent() {
    ...
    let input = context(
        vec![next, event("current", Some("next"), "continue"), old],
        "continue",
    );
...
fn flat_history_is_scoped_and_stops_at_the_current_event() {
    ...
    input
        .events
        .insert(1, event("unassigned", None, "/use-skill unrelated"));
...
fn a_pending_prompt_uses_only_a_unique_scoped_graph_leaf() {
    ...
    input
        .events
        .push(event("other-leaf", Some("root"), "/use-skill beta"));
    assert!(matches!(
        resolve(&input),
        AnchorResolution::MissingTaskContext { .. }
    ));
...
fn cycles_conflicts_and_foreign_current_ids_do_not_fall_back_to_flat_history() {
    ...
    let input = context(
        vec![event("prior", None, "Fix the compiler"), foreign],
        "continue",
    );
    assert!(matches!(
        resolve(&input),
...
fn flat_pending_prompt_and_empty_prompt_only_context_keep_working() {
    ...
    assert_eq!(
        resolve(&input).anchor().unwrap().directives[0].target,
        "alpha"
    );
    let input = context(Vec::new(), "Explain Rust lifetimes");
    assert_eq!(
        resolve(&input).anchor().unwrap().text,
        "Explain Rust lifetimes"
    );
    assert!(matches!(
        resolve(&context(Vec::new(), "continue")),
        AnchorResolution::MissingTaskContext { .. }
    ));
}
        ...
        let mut input = context(
            vec![
                ...
                event(
                    "sibling",
                    Some("root"),
                    "/use-skill nonexistent-sibling-skill",
                ),
                event("current", Some("root"), "continue"),
            ],
            "continue",
