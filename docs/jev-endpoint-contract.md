...
- Insecure `http://` is permitted **only** for loopback addresses in local test/development harnesses:
  ...
  - IPv6 loopback: `::1` (bracketed `[::1]` or unbracketed `::1`).
...
## 4. Rejection of Userinfo, Queries, and Fragments
...
Error messages and diagnostic displays never echo private tokens or input strings.
...
1. **No Insecure Transmission**: Binding an `ApiCredential` to an unencrypted HTTP origin
   (even loopback HTTP) fails immediately with `CredentialRoutingError::InsecureHttpForbidden`.
...
- If a redirect `Location` header is logged in diagnostics, it is first passed through
  `sanitize_url_for_diagnostics`, which strips userinfo credentials and query strings.
...
When inspecting the environment, `ProxyPolicy` scrubs any userinfo or query strings
