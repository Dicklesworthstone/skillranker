...
//!
//! A second counter with the same rules holds invocations the hook found to be
//! a task notification inside a turn already under way (sr-jdji), which are
//! not turns and so never record a row.
...
pub const HOOK_NON_TURNS_FILE: &str = "hook-non-turns.log";
/// About 19,000 entries: months of hook traffic for one person.
pub const HOOK_ENTRIES_MAX_BYTES: u64 = 1024 * 1024;

/// Which counter a record belongs to. Both use the same format and rules.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum HookCounter {
    /// Every hook invocation, counted at entry before stdin is read.
    Entries,
    /// Invocations the hook found to be a harness notification delivered
    /// inside a turn already under way (sr-jdji). They are not turns, so they
    /// never record a row, and `sr stats` subtracts them from `unrecorded`.
    NonTurns,
}
...
impl HookCounter {
    fn file(self) -> &'static str {
        match self {
            Self::Entries => HOOK_ENTRIES_FILE,
            Self::NonTurns => HOOK_NON_TURNS_FILE,
        }
    }
}
...
/// Count this hook invocation in `counter`, beside the ledger the hook records
/// to: `explicit` (`--dir`) or the platform default. Silent on every failure.
pub fn record_hook_invocation(explicit: Option<&Path>, counter: HookCounter) {
    let Some(dir) = explicit
        .map(Path::to_path_buf)
        .or_else(|| super::default_ledger_directory().ok())
    else {
        return;
    };
    let dir = super::storage_path(dir);
    ...
    if dir.is_absolute() {
        record_hook_entry(&dir, super::LEDGER_FILE, counter, now);
    }
}

/// Count this hook invocation in `counter`. Silent on every failure: the
/// counter never changes what the hook does.
pub fn record_hook_entry(dir: &Path, database_file: &str, counter: HookCounter, now_unix_ms: i64) {
    let _ = try_record(dir, database_file, counter, now_unix_ms);
}
...
pub fn count_hook_entries(
    ...
    let Some(bytes) = read_entries(dir, counter)? else {
...
pub fn prune_hook_entries(
    ...
    if removed > 0 {
        replace_entries(dir, counter, &kept)?;
    }
    Ok(removed)
}
...
/// records there were.
pub fn clear_hook_entries(dir: &Path, counter: HookCounter) -> Result<u64, StoreError> {
    let Some(bytes) = read_entries(dir, counter)? else {
        return Ok(0);
    };
    let records = bytes.len().div_ceil(RECORD_LEN) as u64;
    if records > 0 {
        replace_entries(dir, counter, &[])?;
    }
    Ok(records)
}
...
fn read_entries(dir: &Path, counter: HookCounter) -> Result<Option<Vec<u8>>, StoreError> {
    let fd = match open(
        &dir.join(counter.file()),
...
fn replace_entries(dir: &Path, counter: HookCounter, bytes: &[u8]) -> Result<(), StoreError> {
    ...
    let written = (|| {
        ...
        file.sync_all()?;
        std::fs::rename(&staged, dir.join(counter.file()))
    })();
    if written.is_err() {
...
mod tests {
    ...
    const E: HookCounter = HookCounter::Entries;
    ...
    fn entries_are_fixed_size_owner_only_and_counted_by_window() {
        ...
        for at in [1_000, 2_000, 3_000] {
            record_hook_entry(&dir, LEDGER, E, at);
        }
        ...
        let count = count_hook_entries(&dir, E, 1_500, 3_000).unwrap();
        ...
        assert_eq!(
            count_hook_entries(&empty, E, 0, i64::MAX).unwrap(),
            HookEntryCount::default()
        );
    }
    ...
    fn nothing_is_written_where_the_hook_could_not_record_rows() {
        ...
        record_hook_entry(&dir, LEDGER, E, 1_000);
        ...
        record_hook_entry(&shared, LEDGER, E, 1_000);
        ...
        record_hook_entry(&linked, LEDGER, E, 1_000);
        assert_eq!(std::fs::metadata(&target).unwrap().len(), 0);
        assert!(count_hook_entries(&linked, E, 0, i64::MAX).is_err());
    }
    ...
    fn a_full_counter_stops_appending_and_says_its_counts_are_lower_bounds() {
        ...
        record_hook_entry(&dir, LEDGER, E, 6_000);
        assert_eq!(file_len(&dir), before, "a full counter refuses the append");
        let count = count_hook_entries(&dir, E, 0, i64::MAX).unwrap();
        assert!(count.full);
        assert_eq!(count.entries, records as u64);
    }

    #[test]
    fn non_turn_deliveries_are_a_separate_counter() {
        let dir = ledger_dir();
        record_hook_entry(&dir, LEDGER, E, 1_000);
        record_hook_entry(&dir, LEDGER, E, 2_000);
        record_hook_entry(&dir, LEDGER, HookCounter::NonTurns, 2_000);
        assert_eq!(count_hook_entries(&dir, E, 0, i64::MAX).unwrap().entries, 2);
        let non_turns = count_hook_entries(&dir, HookCounter::NonTurns, 0, i64::MAX).unwrap();
        assert_eq!(non_turns.entries, 1);
        assert_eq!(non_turns.first_entry_unix_ms, Some(2_000));
        assert_eq!(
            prune_hook_entries(&dir, HookCounter::NonTurns, 3_000).unwrap(),
            1
        );
        assert_eq!(count_hook_entries(&dir, E, 0, i64::MAX).unwrap().entries, 2);
    }
    ...
    fn prune_and_clear_follow_the_ledger() {
        ...
        assert_eq!(
            count_hook_entries(&dir, E, 0, i64::MAX).unwrap().unreadable,
            1
        );
        assert_eq!(prune_hook_entries(&dir, E, 2_500).unwrap(), 2);
        let count = count_hook_entries(&dir, E, 0, i64::MAX).unwrap();
        ...
        assert_eq!(clear_hook_entries(&dir, E).unwrap(), 2);
        assert_eq!(
            count_hook_entries(&dir, E, 0, i64::MAX).unwrap(),
            HookEntryCount::default()
        );
        // Appends resume after a clear.
        record_hook_entry(&dir, LEDGER, E, 4_000);
        assert_eq!(count_hook_entries(&dir, E, 0, i64::MAX).unwrap().entries, 1);
    }
}
