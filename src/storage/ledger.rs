...
impl LedgerStore {
    ...
    ///
    /// The supplied name is not lost by resolving it: `SnapshotMember` retains
    /// `skill_id` alongside `invocation_name`, so the mapping stays recoverable from the
    /// same snapshot that resolved it.
