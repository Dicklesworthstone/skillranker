...
fn response(connection: &Connection) -> Option<Vec<u8>> {
    connection
        .query_row("SELECT response_bytes FROM sr_response_cache", [], |row| row.get(0))
        .optional()
        .unwrap()
}
...
fn memory_completion_is_single_use_and_refresh_requires_a_new_fence() {
    ...
    assert_eq!(
        coordinator.complete_and_publish(key(), first.owner_token, first.fencing_generation, 110, publish),
        Ok(PublishOutcome::Published)
    );
    assert!(matches!(
        coordinator.complete_and_publish(key(), first.owner_token, first.fencing_generation, 111, publish),
    ...
    assert!(matches!(
        coordinator.complete_and_publish(key(), first.owner_token, first.fencing_generation, 114, publish),
        Ok(PublishOutcome::Superseded { .. })
    ));
    assert_eq!(
        coordinator.complete_and_publish(key(), second.owner_token, second.fencing_generation, 115, publish),
        Ok(PublishOutcome::Published)
    );
...
fn sqlite_completion_preserves_the_first_response_until_a_new_fence_wins() {
    ...
    assert_eq!(publish(&mut connection, &first, 110, b"first"), Ok(PublishOutcome::Published));
    ...
    let second = leading(transact(&mut connection, |tx| {
    ...
    assert_eq!(publish(&mut connection, &second, 114, b"second"), Ok(PublishOutcome::Published));
...
fn failed_sqlite_response_write_does_not_complete_the_lease() {
    ...
    assert!(publish(&mut connection, &leader, 110, b"larger than the fixture response limit").is_err());
    assert_eq!(lease_image(&connection), before);
    assert_eq!(response(&connection), None);
    assert_eq!(publish(&mut connection, &leader, 111, b"retry"), Ok(PublishOutcome::Published));
}
...
fn failed_memory_callback_does_not_complete_the_lease() {
    ...
    assert!(coordinator.complete_and_publish(key(), leader.owner_token, leader.fencing_generation, 110, || {
...
fn backwards_time_never_acquires_refreshes_or_publishes() {
    ...
    assert_eq!(coordinator.complete_and_publish(key(), leader.owner_token, leader.fencing_generation, 99, || {
    ...
    let leader = acquire(&mut connection, 100);
    let before = lease_image(&connection);
    assert!(matches!(transact(&mut connection, |tx| {
...
fn expiry_is_exclusive_in_both_backends() {
    for now in [199, 200, 201] {
        ...
        let memory = coordinator.complete(key(), leader.owner_token, leader.fencing_generation, now).unwrap();
...
fn invalid_expiry_is_refused_before_creating_state() {
    for (now, ttl) in [(100, 0), (i64::MAX as u64, 1), (u64::MAX, 1), (1, u64::MAX)] {
        let invalid = CoordinationPolicy { lease_ttl_ms: ttl, ..policy() };
        let coordinator = MemoryCoordinator::new();
        assert!(matches!(
        ...
        assert!(SqliteLeaseCoordinator::check_lease_on_connection(&connection, key()).unwrap().is_none());
    }
...
fn exhausted_generation_never_wraps_or_replaces_the_owner() {
    ...
    coordinator.leases.write().unwrap().get_mut(&key()).unwrap().fencing_generation = FencingGeneration(i64::MAX as u64);
    let before = coordinator.check_lease(key()).unwrap();
    ...
    connection.execute("UPDATE sr_coordination_leases SET fencing_generation=?1", [i64::MAX]).unwrap();
    let before = lease_image(&connection);
    assert!(transact(&mut connection, |tx| {
...
fn all_sqlite_lease_paths_reject_corruption_without_repair() {
    for mutation in [
        ...
        let mut connection = database();
        let leader = acquire(&mut connection, 100);
        connection.execute(&format!("UPDATE sr_coordination_leases SET {mutation}"), []).unwrap();
        let before = lease_image(&connection);
        assert!(SqliteLeaseCoordinator::check_lease_on_connection(&connection, key()).is_err(), "{mutation}");
        assert!(transact(&mut connection, |tx| {
            SqliteLeaseCoordinator::acquire_in_transaction(tx, key(), 201, &policy())
        }).is_err(), "{mutation}");
        assert!(transact(&mut connection, |tx| {
        ...
        assert!(publish(&mut connection, &leader, 110, b"corrupt").is_err(), "{mutation}");
...
fn active_fence_validation_remains_strict_after_shared_row_admission() {
    ...
    for (now, expected) in [(99, false), (100, true), (199, true), (200, false)] {
        assert_eq!(transact(&mut connection, |tx| {
...
fn fresh_reopen_cannot_replay_a_completed_publication() {
    ...
    let root = std::env::temp_dir().join(format!("sr-lease-integrity-{}-{nonce}", std::process::id()));
    ...
    let leader = acquire(&mut connection, 100);
    assert_eq!(publish(&mut connection, &leader, 110, b"durable"), Ok(PublishOutcome::Published));
    drop(connection);
    let mut reopened = Connection::open(&path).unwrap();
    assert!(SqliteLeaseCoordinator::check_lease_on_connection(&reopened, key()).unwrap().unwrap().is_completed);
    assert!(matches!(
